...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.float32, shape=(
            self.config.batch_size, self.config.n_features), name='input')
        self.labels_placeholder = tf.placeholder(tf.int32, shape=(
            self.config.batch_size, self.config.n_classes), name='label')
        # END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        """
        # YOUR CODE HERE
        W = tf.get_variable(name='weight', shape=(self.config.n_features, self.config.n_classes), initializer=tf.zeros_initializer())
        b = tf.get_variable(name='bias', shape=(self.config.n_classes,), initializer=tf.zeros_initializer())
